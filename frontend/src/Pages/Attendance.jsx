                        ...
                        {loading ? (
                          ...
                          attendance.map((presence, index) => (
                            <tr key={index}>
                              ...
                              <td>{presence.date ? new Date(presence.date).toLocaleDateString("en-GB") : "20/03/25"}</td>
                              <td>{presence.user_entry_time || "10:30 AM"}</td>
                              <td>{presence.user_exit_time || "7:15 PM"}</td>
                              <td>
                                <span className={`badge ${presence.status === 'Absent' ? 'bg-danger' :
                                    presence.status === 'WorkFromHome' ? 'bg-warning text-dark' :
                                      presence.status === 'HalfDay' ? 'bg-primary' :
                                        'bg-success'
                                  }`}>
